...
!!! warning "Do not use environment variables in keystore path" You **cannot use environment variables** in your keystore path in your Xamarin project! The keystore path has to be relative to the folder of your Xamarin Android project. You can also use an absolute path.
...
!!! attention Don't forget to gitignore your keystore in your repository.
...
4. Type your environment variable name here in the first field.
   ...
   !!! example In our example, we use `XAMARIN_KEYSTORE`.
...
5. Upload your keystore file in the `Upload file field`.
   ...
   ![Screenshot](https://github.com/OrganizationDummy/devcenter/tree/acf5f40e38b6dcf6fe62e839a4c04acb31fdebd2/img/android-code-signing/download-url.png)
...
9. Set your keystore path in the `Download destination path`.
   ...
   ![Screenshot](https://github.com/OrganizationDummy/devcenter/tree/acf5f40e38b6dcf6fe62e839a4c04acb31fdebd2/img/android-code-signing/file-downloader.png)

!!! important "Ensure the same keystore path" Ensure that your Bitrise keystore path is the same as your local path relative to your project root. For example, if `keystore.jks`is in your project's root, then your Bitrise keystore path has to be `$BITRISE_SOURCE_DIR/keystore.jks`.
